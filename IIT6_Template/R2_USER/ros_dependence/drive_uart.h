...
typedef uint32_t (*usart_call_back)(int8_t *buf, uint16_t len);
...
void Uart_Init(UART_HandleTypeDef *huart, int8_t *Rxbuffer, uint16_t len, usart_call_back call_back_fun);
...
int8_t sum_crc8(int8_t *data, uint8_t len);
#ifdef __cplusplus
}
#endif
#endif
