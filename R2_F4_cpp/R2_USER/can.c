...
void CAN1_Filter_Init(void)
{
  ...
  sFilterConfig.FilterIdHigh         = (((uint32_t)CAN_RxExtId<<3)&0xFFFF0000)>>16;				
  sFilterConfig.FilterIdLow          = (((uint32_t)CAN_RxExtId<<3)|CAN_ID_EXT|CAN_RTR_DATA)&0xFFFF; 
...
// åå»ºä¸ä¸ªç»æä½å®ä¾ï¼ç¨äºå­å¨ææ°ç CAN1 æ¶æ¯
CAN_RX_Message_t latestCAN1_Message;
...
CAN_RX_Message_t latestCAN1_Message; //接收CAN1数据结构体
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{		
>>>>>>> 944f7e49b9ca7249e370900b25af451d08e604c0
	if(hcan == &hcan1)
	{
	  CAN_RxHeaderTypeDef	RxHeader;      
		Rx_Flag =1;  
		HAL_CAN_GetRxMessage(&hcan1, CAN_RX_FIFO0, &latestCAN1_Message.RxHeader, latestCAN1_Message.RxData);
		//m3508_update_m3508_info(&RxHeader,RxData);  
		// M3508Âµç»ºÊ½Â¾Ý´Â¦Ã?í?		for(m=0;m<8;m++){
		//M3508AngleIntegral(&MOTOR_REAL_INFO[m]);
		//}
	}
