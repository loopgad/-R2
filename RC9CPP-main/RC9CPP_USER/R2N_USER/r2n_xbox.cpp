...
void xbox_r2n::process_data()
{
    ...
    if (speed_level == 0)
    {
        MAX_ROBOT_SPEED_X = 0.50f;
        MAX_ROBOT_SPEED_Y = 0.50f;
        MAX_ROBOT_SPEED_W = 1.20f;
    }
    if (speed_level == 2)
    {
        MAX_ROBOT_SPEED_X = 1.86f;
        MAX_ROBOT_SPEED_Y = 1.86f;
        MAX_ROBOT_SPEED_W = 3.68f;
    }
...
//    }
		...
		if(if_point_track_flag != 1 && robot_stop_flag != 1){
			if(world_robot_flag == 1){
				control_chassis->switch_chassis_mode(remote_worldv);
				control_chassis->setworldv(MAX_ROBOT_SPEED_X * xbox_msgs.joyLHori_map, MAX_ROBOT_SPEED_Y * xbox_msgs.joyLVert_map, -MAX_ROBOT_SPEED_W * xbox_msgs.joyRHori_map);
        ...
        else if(if_point_track_flag == 1 && robot_stop_flag != 1){
